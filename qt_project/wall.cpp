...
Wall::Wall(QPolygonF walls_of_rooms, double height, double thickness)
    : _walls_of_rooms(std::move(walls_of_rooms)), _height(std::move(height)),
      _thickness(std::move(thickness)) {}

QPolygonF Wall::walls() { return _walls_of_rooms; }

double Wall::height() const { return _height; }

double Wall::thickness() const { return _thickness; }
...
void Wall::addWindow(Window *window) { _windows.push_back(window); }

void Wall::addDoor(Door *door) { _doors.push_back(door); }

QVector<Window *> Wall::getWindows() { return _windows; }

QVector<Door *> Wall::getDoors() { return _doors; }
...
bool Wall::containsPoints(QLineF &l2) {
  ...
  while (start != end - 1) {
    ...
    if (angle_is_wrong(l1, l2) || p1.containsPoint(l1.p1(), Qt::FillRule()) ||
        p1.containsPoint(l1.p2(), Qt::FillRule())) {
      start++;
      continue;
    }
    QPointF p;
    if ((p2.containsPoint(l2.p1(), Qt::FillRule()) ||
         p2.containsPoint(l2.p2(), Qt::FillRule()))) {
      return true;
    }
...
QVector<GLfloat> Wall::wall(QPointF p1, QPointF p2, GLfloat c, float h) {
  ...
  QVector<GLfloat> array{
      p2x, h, p2y, p2x, 0, p2y, p1x, 0, p1y,
      p1x, h, p1y, p2x, h, p2y, p1x, 0, p1y,
  };

  return array;
}
void Wall::generateWallsForView(QVector<GLfloat> &vertices,
                                QVector<GLfloat> &colors, QColor color) {
...
QVector<GLfloat> Wall::roof(QPointF p1, QPointF p2) {
  ...
  return {mx,  (GLfloat)(_height + 1.5) / 2, my, p2x, (GLfloat)_height / 2, p2y,
          p1x, (GLfloat)_height / 2,         p1y};
}
void Wall::generateRoof(QVector<GLfloat> &vertices,
                        QVector<GLfloat> &roof_color) {
  auto *start = _walls_of_rooms.begin();
  auto *end = _walls_of_rooms.end() - 1;
...
void Wall::generateHouseWalls(QVector<QVector<GLfloat>> &vert,
                              QVector<QVector<GLfloat>> &uvss) {
  ...
  auto *decend = _decorative_wall.end();
  while (decstart != decend) {
    QVector<GLfloat> wallie = wall((*decstart)->walls()[0],
                                   (*decstart)->walls()[1], 20.0, 3 * _height);
    for (auto nzu : wallie)
      v.push_back(nzu);
...
bool Wall::canMakeDecorativeWall(QVector<QPointF> line) {
  ...
  while (start != end - 1) {
    ...
    QPointF p;
    if (angle_is_wrong(l1, l2) &&
        (l1.intersect(l2, &p) == QLineF::BoundedIntersection ||
         p2.containsPoint(l2.p1(), Qt::FillRule()) ||
         p2.containsPoint(l2.p2(), Qt::FillRule())))
      return true;
...
void Wall::addDecorativeWall(Wall *wall) { _decorative_wall.push_back(wall); }
