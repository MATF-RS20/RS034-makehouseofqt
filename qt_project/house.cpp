...
House::House(QVector<Floor *> floors) : _floors(std::move(floors)) {
  ...
  connect(this->sf, SIGNAL(sendingChoice(int)), this,
          SLOT(changingTextures(int)), Qt::DirectConnection);
...
void House::initializeGL() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    texture[i] = new QOpenGLTexture(
        QImage(QApplication::applicationDirPath() + tex[i]).mirrored());
  ...
  QVector<GLfloat> vertices = {
      ...
      -100.0F, 0.0F,    -100.0F, 100.0F, 0.0F,    -100.0F, -100.0F,
      0.0F,    100.0F,  100.0F,  0.0F,   -100.0F, 100.0F,  0.0F,
      100.0F,  -100.0F, 0.0F,    100.0F

  };
  QVector<GLfloat> uvs = {-50.0F, -50.0F, 50.0F, -50.0F, -50.0F, 50.0F, 50.0F,
...
void House::paintEvent(QPaintEvent *event) {
  ...
  view.lookAt(QVector3D(mid.x() + x, y, mid.y() + z),
              QVector3D(mid.x() + x + lx, y, mid.y() + z + lz),
              QVector3D(0, 1, 0));
...
void House::initializeAgain() {
  for (int i = 0; i < _vertices.size(); i++) {
    ...
    _vbo_vertices.last()->allocate(_vertices.at(i).begin(),
                                   _vertices.at(i).size() * sizeof(GLfloat));
    ...
    _vbo_uvs.last()->bind();
    _vbo_uvs.last()->allocate(_uvs.at(i).begin(),
                              _uvs.at(i).size() * sizeof(GLfloat));
  }
  angle = 0.0;
  lx = 0.0F, lz = -1.0F;
