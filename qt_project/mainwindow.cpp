...
MainWindow::MainWindow(QWidget *parent) : QWidget(parent) {
  ...
  connect(this->drawingarea, SIGNAL(have_points(int)), this,
          SLOT(setPointsnumber()), Qt::DirectConnection);
  connect(this->buttonReset, SIGNAL(clicked()), this->drawingarea,
          SLOT(reset_drawing()), Qt::DirectConnection);
  connect(this->buttonReset, SIGNAL(clicked()), this, SLOT(reseting()));
  connect(this->buttonWallsView, SIGNAL(clicked(bool)), this,
          SLOT(on_buttonWallsView_clicked()));
  connect(this->buttonHouse, SIGNAL(clicked(bool)), this,
          SLOT(on_buttonHouse_clicked()));
  connect(this->buttonComplete, SIGNAL(clicked()), this->drawingarea,
          SLOT(complete_drawing()), Qt::DirectConnection);
  connect(this->drawingarea, SIGNAL(have_been_reseting(QString)),
          this->statusBar, SLOT(setText(QString)), Qt::AutoConnection);
  connect(this->drawingarea, SIGNAL(send_polygon_size(QString)),
          this->statusBar, SLOT(setText(QString)), Qt::DirectConnection);
  connect(this->drawingarea, SIGNAL(update_status_finish(QString)),
          this->statusBar, SLOT(update()), Qt::DirectConnection);
  connect(this->drawingarea, SIGNAL(update_status_finish(QString)),
          this->statusBar, SLOT(setText(QString)), Qt::DirectConnection);
  connect(this->drawingarea, SIGNAL(update_status(QString)), this->statusBar,
          SLOT(setText(QString)), Qt::DirectConnection);
  connect(this->drawingarea, SIGNAL(enableCompleteDrawing(bool)),
          this->buttonComplete, SLOT(setEnabled(bool)), Qt::DirectConnection);
  connect(this, SIGNAL(grabMouse()), this->drawingarea, SLOT(changeOnMouse()),
          Qt::DirectConnection);
  connect(this->drawingarea, SIGNAL(HoverLeave(QPointF &, QPointF)),
          this->drawingarea, SLOT(changeOnMouse()), Qt::DirectConnection);
  connect(this, SIGNAL(enterEvent_()), this->drawingarea, SLOT(changeOnMouse()),
          Qt::DirectConnection);
  connect(this->drawingarea, SIGNAL(fail_to_add_peak(QString)), this->statusBar,
          SLOT(setText(QString)), Qt::DirectConnection);
...
void MainWindow::on_buttonHouse_clicked() {
  ...
  if (!floors.empty()) {
    ...
    QMessageBox::warning(this, tr("Upozorenje"),
                         tr("Niste nacrtali ili niste sacuvali Vas rad"
